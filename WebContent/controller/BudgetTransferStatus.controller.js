...
sap.ui.controller("zhmmaim.controller.BudgetTransferStatus", {
	onInit: function() {
		...
		oModel.read("IMBudgetStatus?", null,
		...
		var oPRStatusTB = this.getView()
				.byId("oPRTable");
		this.getView().byId("S_ABJHR").setHeaderSpan([8,1]);  //Victor 10.31.2016 for Multi Header
		this.getView().byId("R_ABJHR").setHeaderSpan([8,1]);  //Victor 10.31.2016 for Multi Header		
	...
	// commented ShowDetail 10/31/2016 Victor 
	...
	toExportCSV : function(oEvent) {
		...
		var sUrl = sServiceUrl+"/IMBudgetStatus?" +
				"$filter= FRDATE eq '" + oFromDate
					+ "' and TODATE eq '" + oToDate
